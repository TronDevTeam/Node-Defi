...
contract BlacklistToken is BasicToken, ERC20, Ownable {
	...
	modifier whenNotBlacklisted(address _sender, address _receiver) {
		require(!isBlackListed[_sender]);
    require(!isBlackListed[_receiver]);
		_;
	}
	...
	function transfer(address _to, uint _value) whenNotBlacklisted(msg.sender, _to) public returns (bool) {
		return super.transfer(_to, _value);
	}

	function transferFrom(address _from, address _to, uint _value) whenNotBlacklisted(_from, _to) public returns (bool) {
		return super.transferFrom(_from, _to, _value);
	}
}
 ...
 *
 */
contract NodeToken is UpgradeableToken, ReleasableToken, PausableToken, BurnableToken, FreezableToken, ApprovableToken, BlacklistToken {
